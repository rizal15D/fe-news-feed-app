...
export default function FeedPage() {
  ...
  const fetchFeed = async (pageNum: number) => {
    try {
      ...
      // Kalau data kurang dari LIMIT, berarti sudah habis
      if (data.length < LIMIT) setHasMore(false);

      // Page 1: replace posts, Page > 1: append posts
      if (pageNum === 1) {
        setPosts(data);
      } else {
  ...
  useEffect(() => {
    ...
    console.log("👀 observing sentinel:", loadMoreRef.current);
  ...
  }, [hasMore, loadingMore]); // ❗ HAPUS 'page'
